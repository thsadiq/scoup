...
seqDetails <- function(seqVector=0){
  ...
  ndLength <- newVector["blength"]; BL <- ifelse(is.na(ndLength), .1, ndLength)
  t3d <- biTree(taxa, BL, xModel); ttext <- paste0("Tree Info: branch.length=", BL)
  names(sites) <- NULL; names(BL) <- NULL
  seqEntry <- list(sites=sites, taxa=taxa, nodes=nNodes, bl=BL, t3=t3d, words=ttext)
