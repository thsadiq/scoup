...
hbInput <- function(hbVector=0){
  ...
  mdtxt <- paste0("Model Parameters: population.size=", popn,"  method=", cptext)
  names(popn) <-NULL; names(vNvS) <-NULL; names(nonVar) <-NULL; names(methd) <-NULL
  modelEntry <- list(psize=popn, vNvS=vNvS, nsynVar=nonVar, meth=methd, words=mdtxt)
