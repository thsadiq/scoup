...
# ><><                           ~~~~~~~~~~~~~~~~~~~~~                           ><>< #
# ><><                  Discrete Changing Landscape Simulation.                  ><>< #
# ><><                           ~~~~~~~~~~~~~~~~~~~~~                           ><>< #
# ><><                               22nd May 2024                               ><>< #
# ><>< ========================================================================= ><>< #
...
discreteInput <- function(defList=list()){
  ...
  rownames(aaParams) <- c("vNvS","nsynVar"); names(aaParams) <- NULL
  names(app) <- NULL; names(nID) <- NULL; names(pnum) <- NULL; names(xMD) <- NULL
...
sitesim.discrete <- function(parameterz, nodeLength, popSize=NA, ntaxa=NA, s01x22=NA){
...
alignsim.discrete <- function(adaptIn, seqIn, modelIn=NULL, filename="seq.nex"){
  ...
  commentText <- paste0(seqIn$w, ";   ", notes); cdnSEQs <- seqDframe(alignment)
  if(!is.na(filename)) empty <- seqWriter(alignment, treeData, commentText, filename)
  return(list(seqs=alignment, DNDS=dnds_matrix, aInfo=commentText, cseq=cdnSEQs))
}
# ><>< ======== ><>< # Example:
# nodeTheta <- matrix(rep(c(1e-04,1e-05),5), 2, byrow=F)
...
# writeLines("\n"); print(summary(c(outz$D))); writeLines("\n"); print(outz$D[,1:6])
