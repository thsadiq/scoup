...
seqWriter <- function(alignmentMatrix, treeInfo=NA, addText="", fileTag=NULL){
  ...
  if(is.null(fileTag)){
    seqFile <- paste0(getwd(), "/cranrSeqs.nex")}else{seqFile <- fileTag}
  ...
  write.table(closeText, seqFile, T, F, row.names=F, col.names=F)
  write.table(treetext, seqFile, T, F, row.names=F, col.names=F)
