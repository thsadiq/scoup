...
wUpdate <- function(omegaInput){
  ...
  if(omegaInput$technique==1) {aaCreate <- aaGauss} else {aaCreate <- aaGamma}
...
sitesim.omega <- function(parameterz, nodeLength, popSize=NA, ntaxa, s01x22=NA){
  wOutput <- wUpdate( parameterz )
  if(parameterz$technique==1) {aaCreate <- aaGauss} else {aaCreate <- aaGamma}
  ...
  for(a4 in seq(1,n_nodes)){
    ...
    for(a6 in seq(1,length(seqVector))){
      ...
      for(a0 in seq(1,2)){
        ...
        new_codon <- branchSimulate(init_codon, nodeLength, wOutput$qFrame)
...
alignsim.omega <- function(adaptIn, seqIn, modelIn=NULL, filename="seq.nex"){
